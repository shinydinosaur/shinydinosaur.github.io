...
Optimization people have a whole other story to tell. It turns out that there IS a nice, differentiable, soft version of a max function. They like to call it a "softmax" (which does make sense), but we will call it a soft-maximum to avoid a naming overload.

A nice explanation (with pictures!) can be found [here](http://www.johndcook.com/blog/2010/01/13/soft-maximum/) but I will provide the gist of it:
